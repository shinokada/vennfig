...
def and_nand(subs=1, size=15, fill_color='skyblue', bg_color='white', font_size=20):
    ...
    example
    and_nand()
    and_nand(subs=2, size=5, fill_color='#f55faa', bg_color='#f2e4eb', font_size=10)
    """
    ...
    if subs == 1 or subs == 2:
        # A AND B
        ax1.set_title("A AND B", fontsize=font_size)
    ...
    if subs == 1 or subs == 3:
        #  A NAND B
        ax2.set_title("A NAND B", fontsize=font_size)
